...
ORIG_FILE =  os.path.dirname(os.path.realpath(__file__)) + "/orig_file.wav"
FIL_FILE = os.path.dirname(os.path.realpath(__file__)) + "/fil_file.wav"

orig_wav = wave.open(ORIG_FILE, "r")
fil_wav = wave.open(FIL_FILE, "w")

fil_wav.setnchannels(1)
fil_wav.setframerate(16000)
fil_wav.setsampwidth(WIDTH)
...
while True:
    # read audio
    try:
        string_audio_data = orig_wav.readframes(CHUNK)
        ...
        fil_wav.writeframes(string_audio_data)
    except :
        break
